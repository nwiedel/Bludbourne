...
public class Entity {
    ...
    public void update(float delta){
        frameTime = (frameTime + delta) % 5; // want to awoid overflow

        // We want the hitbox at the feet for a better feel
    ...
    public void setBoundingBoxSize(float percentageWithReduced, float percentageHeightReduced){
        // update the current bounding box
        float width;
        float height;

        float widthReductionAmount = 1.0f - percentageWithReduced; // 0.8f for 20% (1 - 0.20)
        float heightReductionAmount = 1.0f - percentageHeightReduced; // 0.8f for 20% (1 - 0.20)
        ...
        // need to account for unit scales, since the map coordinates are in pixels
        float minX;
        float minY;
    ...
    private void loadAllAnimations(){
        ...
        for (int i = 0; i < 4; i++){
            for (int j = 0; j < 4; j++){
                ...
                if(region == null){
                    Gdx.app.debug(TAG, "Ther is no animation frame " + i + "," + j);
                }
